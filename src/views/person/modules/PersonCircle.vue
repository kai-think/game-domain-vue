  ...
  computed: {
    ...
    postProvider() {
      if (this.user && this.user.id)
        return ({current, size}) => bbsApi.page({current, size, userId: this.user.id, status: 1})

      return null
    },
    postReplyProvider() {
      if (this.user && this.user.id)
        return ({current, size}) => bbsApi.pageReply({current, size, userId: this.user.id, status: 0})

      return null
    },
    articleProvider() {
      if (this.user && this.user.id)
        return ({current, size}) => articleApi.page({current, size, userId: this.user.id, status: 1})

      return null
    },
    articleReplyProvider() {
      if (this.user && this.user.id)
        return ({current, size}) => articleReplyApi.page({current, size, userId: this.user.id, descByReplyTime: true, status: 0})
